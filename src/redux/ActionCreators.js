...
export const addWorker = (worker) => ({
  ...
  export const postWorker = (naam,experience,phno,gender,area,specialization) => (dispatch) => {
      const newWorker = {
        naam: naam,
      ...
      return fetch(baseUrl + 'workers', {
        ...
        .then(response => dispatch(addWorker(response)))
        .catch(error => console.log('Post worker ', error.message));
